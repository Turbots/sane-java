...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_escape.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_escape(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_escape? {
            context.push("escape")
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_escape(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
