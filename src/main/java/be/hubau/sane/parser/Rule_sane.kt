...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_sane.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_sane(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_sane? {
            context.push("sane")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_expression.parse(context)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_sane(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
