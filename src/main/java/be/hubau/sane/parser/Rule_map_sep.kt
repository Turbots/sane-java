...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_map_sep.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_map_sep(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_map_sep? {
            context.push("map-sep")
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_map_sep(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }

            context.pop("map-sep", parsed)

            return rule as Rule_map_sep?
        }
    }
}
