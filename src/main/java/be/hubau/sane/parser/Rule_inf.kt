...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_inf.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_inf(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_inf? {
            context.push("inf")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x69.6e.66", "(\\x69\\x6e\\x66)", 3)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_inf(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
