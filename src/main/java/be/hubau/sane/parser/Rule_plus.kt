...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_plus.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_plus(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_plus? {
            context.push("plus")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x2B", "[\\x2B]", 1)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_plus(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
