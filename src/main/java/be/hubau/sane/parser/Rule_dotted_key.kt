...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_dotted_key.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_dotted_key(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_dotted_key? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_simple_key.parse(context)
                        f1 = rule != null
                        if (f1) {
                    ...
                    while (i1 < 1 && f1) {
                        ...
                        run {
                            ...
                            if (parsed) {
                                ...
                                var i2 = 0
                                while (i2 < 1 && f2) {
                                    val rule = Rule_dot_sep.parse(context)
                                    f2 = rule != null
                                    if (f2) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_dotted_key(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_dotted_key?
        }
    }
}
