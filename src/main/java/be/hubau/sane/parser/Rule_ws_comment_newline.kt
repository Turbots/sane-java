...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_ws_comment_newline.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_ws_comment_newline(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_ws_comment_newline? {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_ws_comment_newline(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_ws_comment_newline?
        }
    }
}
