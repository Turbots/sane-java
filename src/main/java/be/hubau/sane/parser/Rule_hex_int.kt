...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_hex_int.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_hex_int(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_hex_int? {
            context.push("hex-int")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_hex_prefix.parse(context)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_hex_int(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }

            context.pop("hex-int", parsed)

            return rule as Rule_hex_int?
        }
    }
}
