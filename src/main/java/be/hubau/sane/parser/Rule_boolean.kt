...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_boolean.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_boolean(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_boolean? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_true.parse(context)
                        f1 = rule != null
                        if (f1) {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_false.parse(context)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_boolean(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
