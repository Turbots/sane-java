...
final public class Rule_ws_comment_newline extends Rule {
    public Rule_ws_comment_newline(String spelling, ArrayList<Rule> rules) {
        super(spelling, rules);
    }

    public static Rule_ws_comment_newline parse(ParserContext context) {
        context.push("ws-comment-newline");
                ...
                while (f1) {
                    ...
                    }
                    {
                        ...
                        if (parsed) {
                            ...
                            int c2 = 0;
                            for (int i2 = 0; i2 < 1 && f2; i2++) {
                                Rule rule = Rule_newline.parse(context);
                                if ((f2 = rule != null)) {
                                    a2.add(rule, context.index);
                                    c2++;
                                }
        ...
        Rule rule = null;
        if (parsed) {
            rule = new Rule_ws_comment_newline(context.text.substring(a0.start, a0.end), a0.rules);
        } else {
            context.index = s0;
        }

        context.pop("ws-comment-newline", parsed);

        return (Rule_ws_comment_newline) rule;
    }
