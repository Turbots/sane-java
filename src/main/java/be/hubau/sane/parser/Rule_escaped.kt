...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_escaped.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_escaped(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_escaped? {
            context.push("escaped")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_escape_seq_char.parse(context)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_escaped(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }

            context.pop("escaped", parsed)

            return rule as Rule_escaped?
        }
    }
}
