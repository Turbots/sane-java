...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_map_open.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_map_open(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_map_open? {
            context.push("map-open")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x7B", "[\\x7B]", 1)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_map_open(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }

            context.pop("map-open", parsed)

            return rule as Rule_map_open?
        }
    }
}
