...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_decimal_point.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_decimal_point(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_decimal_point? {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_decimal_point(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_decimal_point?
        }
    }
}
