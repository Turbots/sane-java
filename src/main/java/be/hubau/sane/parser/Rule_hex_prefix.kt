...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_hex_prefix.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_hex_prefix(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_hex_prefix? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x30.78", "(\\x30\\x78)", 2)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_hex_prefix(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_hex_prefix?
        }
    }
}
