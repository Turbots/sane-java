...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_DIGIT.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_DIGIT(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_DIGIT? {
            context.push("DIGIT")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x30-39", "[\\x30-\\x39]", 1)
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_DIGIT(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
