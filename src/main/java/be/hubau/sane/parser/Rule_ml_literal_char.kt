...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_ml_literal_char.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_ml_literal_char(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_ml_literal_char? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalNumericValue.parse(context, "%x20-7E", "[\\x20-\\x7E]", 1)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_ml_literal_char(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_ml_literal_char?
        }
    }
}
