...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_map.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_map(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_map? {
            context.push("map")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_map_open.parse(context)
                        f1 = rule != null
                        if (f1) {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_map_close.parse(context)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_map(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }

            context.pop("map", parsed)

            return rule as Rule_map?
        }
    }
}
