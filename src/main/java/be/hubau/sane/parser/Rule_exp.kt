...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_exp.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_exp(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_exp? {
            context.push("exp")
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = TerminalStringValue.parse(context, "e")
                        f1 = rule != null
                        if (f1) {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_exp(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
