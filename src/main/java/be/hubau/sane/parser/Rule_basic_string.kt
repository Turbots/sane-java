...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_basic_string.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_basic_string(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_basic_string? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var c1 = 0
                    while (f1) {
                        val rule = Rule_basic_char.parse(context)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_basic_string(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_basic_string?
        }
    }
}
