...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_oct_int.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_oct_int(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_oct_int? {
            ...
            run {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_oct_prefix.parse(context)
                        f1 = rule != null
                        if (f1) {
                ...
                if (parsed) {
                    ...
                    var i1 = 0
                    while (i1 < 1 && f1) {
                        val rule = Rule_digit0_7.parse(context)
                        f1 = rule != null
                        if (f1) {
                            a1.add(rule, context.index)
                            c1++
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_oct_int(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
