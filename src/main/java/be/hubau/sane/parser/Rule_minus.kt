...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_minus.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_minus(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_minus? {
            context.push("minus")
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_minus(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
