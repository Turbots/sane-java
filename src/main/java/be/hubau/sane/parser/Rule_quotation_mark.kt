...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_quotation_mark.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_quotation_mark(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_quotation_mark? {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_quotation_mark(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_quotation_mark?
        }
    }
}
