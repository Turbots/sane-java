...
/* -----------------------------------------------------------------------------
 * be.hubau.sane.parser.Rule_underscore.java
 * -----------------------------------------------------------------------------
 *
...
class Rule_underscore(spelling: String, rules: ArrayList<Rule>) : Rule(spelling, rules) {
    ...
    companion object {
        ...
        fun parse(context: ParserContext): Rule_underscore? {
            ...
            var rule: Rule? = null
            if (parsed) {
                rule = Rule_underscore(context.text.substring(a0.start, a0.end), a0.rules)
            } else {
                context.index = s0
            }
            ...
            return rule as Rule_underscore?
        }
    }
}
